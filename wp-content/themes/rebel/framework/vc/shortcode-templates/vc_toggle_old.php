...
extract(shortcode_atts(array(
    'title' => __("Click to toggle", "mthemes"),
    'el_class' => '',
    'open' => 'false',
    'css_animation' => ''
), $atts));
...
$output .= apply_filters('wpb_toggle_heading', '<h4 class="'.$css_class.' mt_toggle_title">'.$title.'</h4>', array('title'=>$title, 'open'=>$open));
$css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_toggle_content'.$el_class, $this->settings['base']);
$output .= '<div class="'.$css_class.' mt_toggle_content">'.wpb_js_remove_wpautop($content, true).'</div>'.$this->endBlockComment('toggle')."\n";
