...
class Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'post_taxonomy' => array(
				...
				// 'teammembers'  => 'teammembers-categories',
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'You are here:',           'mthemes' ),
			'home'                => __( 'Home',                    'mthemes' ),
			'search'              => __( 'Search results for "%s"', 'mthemes' ),
			'error_404'           => __( '404 Not Found',           'mthemes' ),
			'paged'               => __( 'Page %d',                 'mthemes' ),
			'archives'            => __( 'Archives',                'mthemes' ),
			'archive_minute_hour' => __( 'g:i a',                   'mthemes' ),
			'archive_minute'      => __( 'Minute %d',               'mthemes' ),
			'archive_hour'        => __( 'g a',                     'mthemes' ),
			'archive_day'         => __( 'd',                       'mthemes' ),
			'archive_week'        => __( 'Week %d',                 'mthemes' ),
			'archive_month'       => __( 'F',                       'mthemes' ),
			'archive_year'        => __( 'Y',                       'mthemes' ),
		//	'edit'                => __( 'Edit',                    'mthemes' ), // @todo Implement edit link
		);
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'mthemes' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'mthemes' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'mthemes' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
